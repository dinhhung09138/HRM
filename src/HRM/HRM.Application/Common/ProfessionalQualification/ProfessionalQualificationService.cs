...
namespace HRM.Application.Common
{
    public class ProfessionalQualificationService : IProfessionalQualificationService
    {
        ...
        private async Task<IResult> CheckCurrentVersion(ProfessionalQualificationModel model)
        {
            if (await _professionalQualificationRepository.IsCurrentVersion(model.Id, model.RowVersion) == false)
            {
