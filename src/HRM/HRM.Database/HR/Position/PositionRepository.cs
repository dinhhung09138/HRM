...
{
    public sealed class PositionRepository : EFRepository<Position>, IPositionRepository
    {
        public PositionRepository(Context context) : base(context) { }

        public async Task<PositionModel> FindByIdAsync(long id)
        {
            return await Queryable.Where(m => m.Id == id && m.Deleted == false).Select(PositionExpression.FindByIdAsync).FirstOrDefaultAsync();
        }

        public async Task<Model.Grid<PositionGridModel>> GridAsync(PositionGridParameterModel paramters)
        {
            ...
            var grid = await query.Select(PositionExpression.GridAsync).GridAsync(paramters);

            var result = new Model.Grid<PositionGridModel>();
        ...
        public async Task<bool> SaveAsync(Position model, bool isCreate)
        {
            if (isCreate == true)
            {
        ...
        public async Task<bool> DeleteAsync(Position model)
        {
            await UpdatePartialAsync(new
            {
                model.Id,
