...
namespace HRM.Database.HR
{
    ...
    {
        public RelationshipTypeRepository(Context context) : base(context) { }

        public async Task<RelationshipTypeModel> FindByIdAsync(long id)
        {
            return await Queryable.Where(m => m.Id == id && m.Deleted == false).Select(RelationshipTypeExpression.FindByIdAsync).FirstOrDefaultAsync();
        }

        public async Task<Model.Grid<RelationshipTypeGridModel>> GridAsync(RelationshipTypeGridParameterModel paramters)
        {
            ...
            var grid = await query.Select(RelationshipTypeExpression.GridAsync).GridAsync(paramters);

            var result = new Model.Grid<RelationshipTypeGridModel>();
        ...
        public async Task<bool> SaveAsync(RelationshipType model, bool isCreate)
        {
            if (isCreate == true)
            {
        ...
        public async Task<bool> DeleteAsync(RelationshipType model)
        {
            await UpdatePartialAsync(new
            {
                model.Id,
