...
namespace HRM.Client.Pages.Hr.Setting.Ranking
{
    partial class RankingForm : ComponentBase
    {
        [Inject]
        ...
        private Form<RankingModel> form;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (Id.HasValue)
            {
                ...
                var result = await httpClientService.Get<RankingModel, HttpDataResponseWrapper<RankingModel>>($"hr/ranking/{Id.Value}");
        ...
        {
            navigationManager.NavigateTo("hr/setting/ranking");
        }
        ...
        protected async Task OnFinish(EditContext editContext)
        {
            ...
            if (Id.HasValue)
            {
                var response = await httpClientService.Put<RankingModel, HttpActionResponseWrapper>($"hr/ranking/{Id.Value}", model);
            ...
            {
                var response = await httpClientService.Post<RankingModel, HttpActionResponseWrapper>("hr/ranking", model);
                if (response.Succeeded)
                {
        ...
        private void DefineBreadcumb()
        {
            ...
            Breadcrumb.Add(new BreadcurmbModel()
            {
                Title = "Danh sách xếp loại học tập",
                Href = "hr/setting/ranking",
                IsActive = false,
            });
