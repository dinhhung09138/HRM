...
{
    partial class EthnicityList : ComponentBase
    {
        [Inject]
        ...
        private List<EthnicityGridModel> listData = new List<EthnicityGridModel>();
        private int totalItems = 0;
        Func<PaginationTotalContext, string> showTotal = ctr => $"Tổng: {ctr.Total} dòng";

        private EthnicityGridParameterModel parameterModel = new EthnicityGridParameterModel();

        private EthnicityGridModel deletedItem = new EthnicityGridModel();
        ...
        {
            navigationManager.NavigateTo("hr/setting/ethnicity/create");
        }

        protected void UpdateClick(EthnicityGridModel item)
        {
            navigationManager.NavigateTo($"hr/setting/ethnicity/update/{item.Id}");
        }

        protected void DeleteClick(EthnicityGridModel item)
        {
            deletedItem = item;
        ...
        {
            var result = await httpClientService.Delete<EthnicityModel, HttpActionResponseWrapper>($"hr/ethnicity/{deletedItem.Id}");
            if (result.Succeeded)
            {
        ...
        private async Task LoadGridData()
        {
            ...
            var result = await httpClientService.Post<EthnicityGridParameterModel, HttpDataResponseWrapper<Model.Grid<EthnicityGridModel>>>("hr/ethnicity/grid", parameterModel);
            if (result != null)
            {
                listData = result.Data.List?.ToList() ?? new List<EthnicityGridModel>();
